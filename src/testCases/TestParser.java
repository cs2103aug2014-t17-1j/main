...
public class TestParser {
	...
	public void testAdd() {
		...
		Assert.assertEquals(CommandType.ADD, result.getCommandType());
		Assert.assertEquals("homework", result.getTaskDetails().getTitle());
		Assert.assertEquals("20/08/2200", result.getTaskDetails().getDueDate().toLocalDate().toString("dd/MM/yyyy"));
		Assert.assertEquals(TaskType.DEADLINE, result.getTaskDetails().getTaskType());
		
		// With Deadline, category, impt, note
		result = testingParser.parseString("add homework -due 20 aug 2200 -category testing -impt Y -note extra points");
		Assert.assertEquals(true, result.getIsExecutorApplicable());
		Assert.assertEquals(CommandType.ADD, result.getCommandType());
		Assert.assertEquals("homework", result.getTaskDetails().getTitle());
		Assert.assertEquals("20/08/2200", result.getTaskDetails().getDueDate().toLocalDate().toString("dd/MM/yyyy"));
		...
		result = testingParser.parseString("add homework -from 20 aug 2200 -to 10 sep 2200");
		Assert.assertEquals(true, result.getIsExecutorApplicable());
		Assert.assertEquals(CommandType.ADD, result.getCommandType());
		Assert.assertEquals("homework", result.getTaskDetails().getTitle());
		Assert.assertEquals("20/08/2200", result.getTaskDetails().getStartDate().toLocalDate().toString("dd/MM/yyyy"));
		Assert.assertEquals("10/09/2200", result.getTaskDetails().getDueDate().toLocalDate().toString("dd/MM/yyyy"));
		Assert.assertEquals(TaskType.TIMED, result.getTaskDetails().getTaskType());
		
		// From startDate to end date, category, impt, note
		result = testingParser	.parseString("add homework -from 20 aug 2200 -to 10 sep 2200 -category Sample -impt Y -note extra points");
		Assert.assertEquals(true, result.getIsExecutorApplicable());
		Assert.assertEquals(CommandType.ADD, result.getCommandType());
		Assert.assertEquals("homework", result.getTaskDetails().getTitle());
		Assert.assertEquals("20/08/2200", result.getTaskDetails().getStartDate().toLocalDate().toString("dd/MM/yyyy"));
		...
		result = testingParser	.parseString("add homework -due today -from 20 aug 2200 -to 10 sep 2200 -category Sample -impt Y -note extra points");
		...
		result = testingParser	.parseString("add homework -from 20 aug 2200 -to 10 sep 2200 -due today -category Sample -impt Y -note extra points");
		Assert.assertEquals(false, result.getIsExecutorApplicable());
		Assert.assertEquals(Constants.MESSAGE_INVALID_COMBINATION_DUE_AND_FROMTO, SummaryReport.getFeedBackMsg());
		
		//Using 'to' command without 'from' command(expected error)
		result = testingParser	.parseString("add homework -to 10 sep 2200 -category Sample -impt Y -note extra points");
		Assert.assertEquals(false, result.getIsExecutorApplicable());
		...
		result = testingParser.parseString("add homework -due 20th aug 2200 -category testing -impt Y -note extra points");
		Assert.assertEquals(false, result.getIsExecutorApplicable());
		...
		result = testingParser.parseString("add homework -from 20th aug 2200 -to 10 sep 2200 -category testing -impt Y -note extra points");
		Assert.assertEquals(false, result.getIsExecutorApplicable());
		Assert.assertEquals(Constants.MESSAGE_INVALID_DATE, SummaryReport.getFeedBackMsg());
	
		// start date,end date, category, impt, note (Put in invalid end date, error expected)
		result = testingParser.parseString("add homework -from 20 aug 2200 -to 10th sep 2200 -category testing -impt Y -note extra points");
		Assert.assertEquals(false, result.getIsExecutorApplicable());
		Assert.assertEquals(Constants.MESSAGE_INVALID_DATE, SummaryReport.getFeedBackMsg());
		
		//start date, end date, category, impt, note (Put in invalid impt level, error expected)
		result = testingParser.parseString("add homework -from 20 aug 2200 -to 10 sep 2200 -category testing -impt anything  -note extra points");
		Assert.assertEquals(false, result.getIsExecutorApplicable());
	...
	public void testDisplay() {
		...
		//range of dates
		result = testingParser.parseString("display 20/08/2200 -to 10 sep 2200");
		Assert.assertEquals(true, result.getIsExecutorApplicable());
		Assert.assertEquals(CommandType.DISPLAY, result.getCommandType());
		Assert.assertEquals("20/08/2200", result.getTaskDetails().getStartDate().toLocalDate().toString("dd/MM/yyyy"));
		Assert.assertEquals("10/09/2200", result.getTaskDetails().getDueDate().toLocalDate().toString("dd/MM/yyyy"));
